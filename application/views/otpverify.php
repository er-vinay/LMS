					...
					<div class="form">
						<form method="post" action="<?= base_url('verifyotp');?>" id="formData" autocomplete="off">
							...
							<div class="form-group">
                                <p class="text-center">
                                    <span style="color : red; font-size : 20px;">*</span>Verify OTP
                                </p>
                                <input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" />
								<input type="text" name="otp" class="form-control" placeholder="Enter OTP" title="OTP" required>
							</div>
							<div class="form-group">
								<button class="form-control" id="userSigin" title="Verify OTP">Create</button>
							</div>
						</form>
